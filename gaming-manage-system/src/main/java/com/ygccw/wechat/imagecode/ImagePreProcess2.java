...
public class ImagePreProcess2 {
    ...
    public static int isDeep(int colorInt) {
        Color color = new Color(colorInt);
        if (color.getRed() + color.getGreen() + color.getBlue() > 350) {
            return 1;
        }
        return 0;
    }
...
//        ImageIO.write(img, "PNG", new File("E:\\验证码\\result\\ValidateCode.png"));
        return img;
    }

    public static BufferedImage removeBackgroudOne(BufferedImage img) {
        int width = img.getWidth();
        int height = img.getHeight();
        for (int x = 0; x < width; ++x) {
            for (int y = 0; y < height; ++y) {
        ...
        try {
            ImageIO.write(img, "PNG", new File("E:\\验证码\\result\\ValidateCode2.png"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        return img;
...
//        }
    ...
    public static String getAllOcr(String file) throws Exception {
        ...
        ImageIO.write(img, "JPG", new File("E:\\验证码\\result" + result + ".jpg"));
        return result;
    }
...
//        Map<BufferedImage, String> map = loadTrainData();
    ...
    public static BufferedImage splitImageWord(BufferedImage img)
        ...
        ImageIO.write(imgNew, "png", new File("E:\\验证码\\result30.png"));
        return null;
    }

    public static void check(BufferedImage img) {
        String result = "";
        try {
            Map<BufferedImage, String> map = new HashMap<BufferedImage, String>();
            File dir = new File("E:\\验证码\\check\\train2");
            File[] files = dir.listFiles();
            for (File file : files) {
                map.put(ImageIO.read(file), file.getName().charAt(0) + "");
            }
