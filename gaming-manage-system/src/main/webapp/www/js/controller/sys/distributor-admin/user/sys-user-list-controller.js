(function (angular) {
    ...
    app.controller('DistributorAdminSysUserListController', ['$scope', '$rootScope', '$http', '$location', '$state', function ($scope, $rootScope, $http, $location, $state) {
        $scope.offset = 0;
        $scope.fetchSize = 25;
        var loadData = function (offset, fetchSize) {
            var url = "/sys-user/staff?offset=" + offset + "&fetchSize=" + fetchSize;
        ...
        $scope.modifyRole = function (userUuid) {
            ...
            $location.path('wechat/sys-user/modify-role');
        }


        $scope.resetPassword = function (id) {
            if (id)$location.search("id", id);
            $location.path('wechat/sys-user/reset-password');
        };

        $scope.toAddOrUpdate = function (id) {
            if (id)$location.search("id", id);
            $location.path('wechat/sys-user/update');
        }
